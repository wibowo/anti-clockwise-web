...
- `index.html` — homepage
- `About.dc.html`, `Consulting.dc.html`, `Contact.dc.html`, `Products.dc.html` — site pages
- `support.js` — runtime that powers the `.dc.html` pages (templating, hover styles, etc.). This file is **generated from `dc-runtime/src/*.ts`** and should not be edited by hand — rebuild it with `cd dc-runtime && bun run build`.
